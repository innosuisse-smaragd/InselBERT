...
class FactExtractionRunnable(bentoml.Runnable):
    ...
    def predict_facts_and_anchors(self,example: str):
        ...
        predicted_tokens, predicted_tags = self.predict_anchors(self.tokenizer, tokenized_input, outputs)
        structured_anchors_dataframe = pd.DataFrame(
            [predicted_tokens, predicted_tags], index=["tokens", "predicted_anchor_tags"]
        )
        ...
        for pred in raw_fact_predictions:
            if len(pred["tags"]) > 0:
                ...
                extracted_fact_predictions.append(0)
        print("Extracted fact predictions: ", extracted_fact_predictions)
        facts_dataframe = pd.DataFrame(extracted_fact_predictions)
        facts_dataframe_transposed = facts_dataframe.transpose()

        print("Facts output: ", facts_dataframe_transposed)
        merged_dataframe = pd.concat([structured_anchors_dataframe, facts_dataframe_transposed], axis=1)
        print("Merged dataframe: ", merged_dataframe)
